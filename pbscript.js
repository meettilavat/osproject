...
$("#tab").on("input", function () {
    ...
    for (let i = 0; i < pro.value ; i++) {
        //select row from which to select values
        ...
        //input validation (check if values are not a number, less than 0 or empty), disable button if validation fails
...
function calculate() {
    ...
    //sort processes based on priority (lower means higher priority)
    sort();
 ...
 */
 ...
 function sort() {
    ...
    for (let i = 0; i < pro.value; i++) {
        pos=i;
        //Compare current process's priority to the next process's priority
